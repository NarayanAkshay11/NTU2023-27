...
IMU_Data imu; // structure of the imu
float pitch_acc; //calculate the pitch
float roll_acc; //calculate the roll acceleration
char sbuf[16][15]; //can store 16 strings with 15 characters? why isthis used specifically
float pitch_gyro = 0;
float roll_gyro = 0 ;
float yaw_gyro = 0;
float pitch_fusion_comp = 0;
float roll_fusion_comp = 0;
float Var_gyro = 16;
float Var_acc = 9;
float roll_var_g = 4;
float pitch_var_g = 4;
...
float roll_KF = 0;
float pitch_KF = 0;
  float dt;   //calculate the change in the time. (old - new)
  uint32_t millisOld, millisNow; // old - new time
...
int main(void)
{
  ...
  //sprintf(&sbuf, "%5.2f , %5.2f , %5.2f  \n\r", imu.acc[0], imu.acc[1], imu.acc[2]);
  ...
  //formatting pitch and roll
  //sprintf(sbuf ,"%5.2f, %5.2f ", pitch_acc, roll_acc );
  //HAL_UART_Transmit(&huart3, sbuf, 10, HAL_MAX_DELAY);
 // HAL_UART_Transmit(&huart3, "\n\r", 2, HAL_MAX_DELAY);
  //HAL_Delay(100);
// //send the data
// HAL_UART_Transmit(&huart3, sbuf, 10, HAL_MAX_DELAY);
//    HAL_UART_Transmit(&huart3, "\r\n", 2, HAL_MAX_DELAY); //newline of the data
...
//     HAL_UART_Transmit(&huart3, "\r\n", 2, HAL_MAX_DELAY); //newline of the data
  ...
  millisOld = millisNow; // update the millisold


  pitch_gyro += imu.gyro[1] * dt ; // Integrate the angular velocity of x * dt
  roll_gyro += imu.gyro[0] * dt ; //Integrate the angular velocity of y  * dt
  yaw_gyro += imu.gyro[2] * dt ;
  ...
  HAL_UART_Transmit(&huart3, "\n\r", 2, HAL_MAX_DELAY);
  HAL_Delay(100);
  //HAL_UART_Transmit(&huart3, sbuf[14][0], 6, HAL_MAX_DELAY);
  //HAL_UART_Transmit(&huart3, sbuf[14][1], 6, HAL_MAX_DELAY);
  //HAL_UART_Transmit(&huart3, "\r\n", 2, HAL_MAX_DELAY); //newline of the data
